...
class DocumentProcessingUseCase:
    ...
    async def process_document(self, document: Document) -> Document:
        """
        Process a document: extract structure, generate embeddings, store in vector DB
        """
        try:
            ...
            try:
                for i, (chunk, embedding) in enumerate(zip(contextual_chunks, embeddings)):
                    ...
                    self.logger.debug(f"Added embedding to chunk {i + 1}/{len(contextual_chunks)} for document {document.id}")
            except Exception as e:
            ...
            self.logger.info(f"Storing {len(contextual_chunks)} chunks in vector database for document {document.id}")
            self.logger.log(1, contextual_chunks, exc_info=True)
            try:
                await self.vector_store.add_chunks(contextual_chunks)
                self.logger.info(f"Successfully stored chunks in vector database for document {document.id}")
            except Exception as e:
                self.logger.error(f"Failed to store chunks in vector database for {document.id}: {str(e)}")
    ...
    async def _add_contextual_information(
        ...
        for chunk in chunks:
            ...
            try:
                ...
                contextual_content = f"{context.strip()} {chunk.content}"
